...
uint32_t KP::ENGINE::Vertex::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
    ...
    vkGetPhysicalDeviceMemoryProperties(*KP::ENGINE::OBJECT_device.getPointerPhysicalDevice(), &memProperties);
...
VkCommandBuffer KP::ENGINE::Vertex::beginSingleTimeCommands() {
    ...
    err = vkAllocateCommandBuffers(*KP::ENGINE::OBJECT_device.getPointerDevice(), &allocInfo, &commandBuffer);
    check_vk_result(err);
 ...
 void KP::ENGINE::Vertex::endSingleTimeCommands(VkCommandBuffer commandBuffer) {
    ...
    vkFreeCommandBuffers(*KP::ENGINE::OBJECT_device.getPointerDevice(), commandPool, 1, &commandBuffer);
}
